...
class HomeController extends Controller
{
  public function index()
  {
    ...
    $top10UserDeposit = Transaction::selectRaw('username, sum(amount) as total')
    ...
    if ($lang === 'vn') {
      foreach ($transactions as $transaction) {
        $listAccountBuy .= "<span style=\"color: #504099\">" . Helper::hideUsername($transaction->username) . "</span> "
          . __t('cách đây') . " <span style=\"color: #E25E3E\">" . Helper::getTimeAgo($transaction->created_at) . "</span> "
          . __t('đã mua tài khoản') . " <span style=\"color: #279EFF\">#" . ($transaction->extras['code'] ?? $transaction['extras']['account_id'] ?? '-') . "</span> - "
          . __t('Giá') . " <span style=\"color: #4D2DB7\">" . Helper::formatCurrency($transaction->amount) . "</span> | \n";
      }
    } else {
      foreach ($transactions as $transaction) {
        $listAccountBuy .= "<span style=\"color: #504099\">" . Helper::hideUsername($transaction->username) . "</span> purchased account "
          . "<span style=\"color: #279EFF\">#" . ($transaction->extras['code'] ?? $transaction['extras']['account_id'] ?? '-') . "</span> "
          . "<span style=\"color: #E25E3E\">" . Helper::getTimeAgo($transaction->created_at) . "</span> for "
          . "<span style=\"color: #4D2DB7\">" . Helper::formatCurrency($transaction->amount) . "</span> | \n";
      }
    }

    $cardOn = theme_config('deposit_card', true);
    ...
    foreach (CardList::where('status', true)->get() as $card) {
      $fees[$card->code] = $card->fees;
    }
