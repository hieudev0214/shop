console.log("info: Hieu IT");
console.log("author: fb.com/ADMIN.PHAMVANHIEU");
console.log("website: Hieu IT");
window.$getResponseMessage = function (e) {
  return e.response && e.response.data
    ? e.response.data.message
    : e.message || "Unknown error";
};
window.$getRequestMessage = function (e) {
  return e.message || "Error in request";
};
window.$getStatusMessage = function (e) {
  return e.responseJSON ? e.responseJSON.message : e.statusText;
};
window.$getErrorMessage = function (e) {
  return e.message || e.stack;
};
window.$catchMessage = function (e) {
  let n = "System error occurred";
  return (
    (n = e.isAxiosError
      ? e.response
      ...
      : e.status
        ? $getStatusMessage(e)
        : $getErrorMessage(e)),
    console.log(e.response || e.request || e),
    n
  );
};
window.$parseError = function (e) {
  try {
    let n = "System error occurred";
    return (
      (n = e.isAxiosError
    ...
    return (console.log(n), "Error occurred while handling error");
  }
};
window.$formatCurrency = function (e, n = "VND", s = 2) {
  return (
...
window.$formatDateTime = function (e, n = "YYYY-MM-DD HH:mm:ss") {
  return moment(e).format(n);
};
window.$formatStatus = function (e) {
  switch (e) {
    case "Running":
    ...
    default:
      return `<span class="badge bg-secondary rounded-lg text-white" style="background-color: #213363">${e}</span>`;
  }
};
window.$setLoading = function (e) {
  $(e).attr("disabled", !0).addClass("process");
};
window.$removeLoading = function (e) {
  $(e).attr("disabled", !1).removeClass("process");
};
window.$formatDate = function (e, n = "YYYY-MM-DD HH:mm:ss") {
  return moment(e).format(n);
};
window.$isURL = function (e) {
  let n =
    /(http|https):\/\/(\w+:{0,1}\w*)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%!\-\/]))?/;
  return new RegExp(n).test(e);
};
window.$swal = function (e, n, s = {}) {
  return Swal.fire({
    icon: e === "success" ? "success" : "error",
    title: e === "success" ? "Thành công" : "Thất bại",
    text: n,
    ...s,
  });
};
window.$showLoading = function (e = null) {
  Swal.fire({
    icon: "info",
    title: "Đang xử lý!",
    html: e ?? "Không được tắt trang này, vui lòng đợi trong giây lát!",
    timerProgressBar: !0,
    allowOutsideClick: !1,
    allowEscapeKey: !1,
    allowEnterKey: !1,
...
window.$getCountryName = function (e) {
  return new Intl.DisplayNames(["en"], { type: "region" }).of(
    e == null ? void 0 : e.toUpperCase(),
  );
};
window.$formDataToPayload = function (e) {
  const n = {};
  for (let [s, t] of e.entries()) n[s] = t;
  return n;
};
...
window.addEventListener("offline", function (e) {
  (console.log("offline"), $swal("error", "Mất kết nối mạng"));
});
function o(e) {
  let n = e.target;
  (n.removeEventListener("error", o), (n.src = "/images/error-404.webp"));
}
document.querySelectorAll("img").forEach((e) => {
  e.naturalWidth === 0 && (e.addEventListener("error", o), (e.src = e.src));
});
var a = new ClipboardJS(".copy");
a.on("success", function (e) {
  $swal("success", "Đã sao chép: " + e.text);
});
a.on("error", function (e) {
  $swal("error", "Sao chép thất bại");
});
window.$userLevelName = function (e) {
  return e.charAt(0).toUpperCase() + e.slice(1);
};
window.$logout = async function () {
  try {
    const e = await axios.post("/logout");
  } finally {
